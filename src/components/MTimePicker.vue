...
<template>
  <div class="flex flex-col gap-1">
    <div ref="triggerEl" class="relative mt-2" :style="{ '--field-bg': resolvedFieldBg }">
      ...
      >
        <MIcon name="schedule" :size="24" class="shrink-0" :class="disabled ? 'text-on-surface/38' : 'text-on-surface-variant'" />
        <span v-if="displayValue" class="flex-1 font-mono" :class="disabled ? 'text-on-surface/38' : 'text-on-surface'">{{ displayValue }}</span>
        <span v-else class="flex-1" :class="disabled ? 'text-on-surface/38' : 'text-on-surface-variant'">{{ label || placeholder || locale.selectTime }}</span>
        <MIcon
          v-if="modelValue"
          name="close"
          :size="18"
