...
- `MSelect`: a `class` (or any other attribute) passed to `<MSelect>` was silently discarded — its template renders a fragment (the field `<div>` plus a sibling `<Teleport>` for the dropdown), and Vue only auto-applies attrs fallthrough to a *single*-root component. Without `inheritAttrs: false` + an explicit `v-bind="$attrs"` on the field div, nothing told Vue which of the two roots should receive it, so it was dropped entirely — a consumer trying `<MSelect class="w-64" />` to give it a fixed width got no error, just a field that silently kept sizing itself off its own content (in practice: it resized every time the selected option's label changed length). Now forwards correctly
